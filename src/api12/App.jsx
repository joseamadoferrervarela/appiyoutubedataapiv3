...
export function App() {
  ...
  const [idchannel,setIdchannel] = useState("eQNHDV7lKgE");
      ...
      Promise.all(respuesta).then(values => {
        ...
        values.map((value,ind)=>{
          ...
          Promise.all(respuestan[ind]).then(valuesn => {
            ...
            const listbox= value.items.map(element=> `<div class='videos'><h1 class='namelist'>${element.snippet.title}</h1><h2 class='descriptionlist'>${element.snippet.description}</h2><img src='${element.snippet.thumbnails.medium.url}'></div>`)
